...
class Optimizer:
    ...
    def __init__(self, graph,target,batch_size=12):
        ...
        :param graph: 一个Graph对象
        :param target: 一个作为优化目标的节点对象
        :param batch_size: Mini Batch的样本数量
        """
    ...
    def one_step(self):
        ...
        :return:
        """
        # 调用forward_backward方法对一个样本执行前向传播和反向传播，
        # 将目标节点对各个变量的梯度累加在acc_gradient中，最后清除所有节点的雅可比矩阵
        self.forward_backward()

        ## 计数样本数，当样本数达到batch_size时，执行update方法更新变量，并清除累加梯度以及计数
        self.acc_no += 1
    ...
    def get_gradient(self,node):
        """
        返回一个Mini Batch的所有样本的平均梯度
        :param node:
        :return:
        """
        # 对一个Mini Batch中的样本依次执行前向传播和反向传播，
        # 将参与训练的变量的梯度累加在acc_gradient中，
        # 一个Mini Batch计算完毕后执行变量更新，这时使用Mini Batch中多个样本的平均梯度
        assert node in self.acc_gradient
        return  self.acc_gradient[node] / self.batch_size


    # update方法将在具体的优化器类中得到实现
    def update(self):
        """
        抽象方法，利用梯度更新可训练变量
        :return:
    ...
    def forward_backward(self):
        """
        前向传播计算结果节点的值，然后反向传播计算结果节点对各个节点的梯度
        :return:
        """
        # 清除图中所有节点的雅可比矩阵
    ...
    def update(self):
        ...
        for node in self.graph.nodes:
            if isinstance(node, Variable) and node.trainable:
                ...
                node.set_value(node.value - self.learning_rate * gradient / gradient (np.sqrt(self.s[node] + 1e-10)))
